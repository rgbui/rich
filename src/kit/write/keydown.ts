    ...
    if (event.key == KeyboardCode.ArrowLeft) {
        ...
        if (aa.isStart(sel.focusNode, sel.focusOffset)) {
            ...
            if (prevAA) {
                ...
                write.kit.cursor.onFocusAppearAnchor(prevAA, { last: prevAA.isBeforeNear(aa) ? -1 : true })
            }
            else {
                //这说明光标处于当前文档的头部
            }
        ...
        if (aa.isEnd(sel.focusNode, sel.focusOffset)) {
            ...
            if (downAA) {
                event.preventDefault();
                write.kit.cursor.onFocusAppearAnchor(downAA, { at: downAA.isAfterNear(aa) ? 1 : 0 })
            } else {
                //说明光标处于文档的尾部
            }
            return;
        ...
        if (Math.abs(rect.bottom - rects.last().bottom) < lineHeight) {
            ...
            if (downAA) {
                ...
                write.kit.cursor.onFocusAppearAnchor(downAA, { left: rect.left, y: rects.last().bottom + lineHeight / 2 })
            }
        }
    }
        ...
        if (Math.abs(rect.top - rects.first().top) < lineHeight) {
            ...
            if (upAA) {
                ...
                write.kit.cursor.onFocusAppearAnchor(upAA, { left: rect.left, last: true, y: rects.first().top + lineHeight / 2 })
            }
        }
    }
}
...
export async function onKeyTab(write: PageWrite, aa: AppearAnchor, event: React.KeyboardEvent) {
    ...
    await InputForceStore(aa, async () => {
        ...
        page.addUpdateEvent(async () => {
            ...
            if (newAA)
                write.kit.cursor.onFocusAppearAnchor(newAA, { merge: true, at: offset });
        })
