        ...
        if (props.block.noTitle) return <div className='h-20 relative'>
            {props.block.isOver && <div className="flex h-40 pos card round padding-w-10 padding-h-0" style={{ top: -30, left: 0, right: 0 }}>
                ...
                <div className="sy-dg-tool-operators flex-auto flex-end">
                    ...
                    {props.block.filter?.items?.length > 0 && <label className="item-hover round size-24 flex-center cursor gap-r-10" onMouseDown={e => props.block.onOpenViewConfig(Rect.fromEvent(e), 'filter')}><Icon size={14} icon={FilterSvg}></Icon><span>过滤</span></label>}
                    {props.block.sorts?.length > 0 && <label className="item-hover round size-24 flex-center cursor gap-r-10" onMouseDown={e => props.block.onOpenViewConfig(Rect.fromEvent(e), 'sort')}><Icon size={14} icon={SortSvg}></Icon><span>排序</span></label>}
                    {props.block.page.pageLayout.type != PageLayoutType.db && <label className="item-hover round size-24 flex-center cursor gap-r-10" onMouseDown={e => { e.stopPropagation(); props.block.onOpenSchemaItem(Rect.fromEvent(e)) }}><Icon icon={MaximizeSvg} size={16}></Icon></label>}
