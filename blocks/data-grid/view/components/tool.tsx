            ...
            {props.block.isOver && <div className="flex h-40 pos card round padding-w-10 padding-h-0" style={{ top: -30, left: 0, right: 0 }}>
                <div className="sy-dg-tool-title">
                    <label className="cursor flex round h-30 item-hover padding-r-5 text" onMouseDown={e => { e.stopPropagation(); props.block.onOpenViewSettings(Rect.fromEvent(e)) }}>
                        <span className="size-30 flex-center flex-fixed">
                ...
                <div className="sy-dg-tool-operators">
                    <label className="item-hover round padding-w-10 h-30 flex-center cursor gap-r-10" onMouseDown={e => props.block.onOpenViewConfig(Rect.fromEvent(e))}><Icon size={14} icon={SettingsSvg}></Icon><span className="f-14 padding-l-5">视图配置</span></label>
