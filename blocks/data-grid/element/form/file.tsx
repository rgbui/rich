...
class FormFieldFileView extends BlockView<FormFieldFile> {
    ...
    renderView() {
        ...
        return <FieldView block={this.block} className={'visible-hover'}>
            ...
            {this.block.fromType == 'doc-add' && this.renderForm(vs)}
            {this.block.fromType == 'doc' && this.renderField(vs)}
        </FieldView>
    }
    renderDetail(files: { filename: string, size: number, url: string }[]) {
        if (files.length == 0) return <div className={"f-14 min-h-30 cursor f-14 flex  remark"}>{<S>空内容</S>}</div>
        console.log(this.block.field, files);
        if (this.block.field?.type == FieldType.file || files.length > 1) {
            return <div>
        ...
        else if (this.block.field?.type == FieldType.video) {
            var VC = lazy(() => import('../../../media/video/player'));
            return <div>
                <Suspense fallback={<Spin block></Spin>}>
                    <VC src={files[0].url} height={250} ></VC>
                </Suspense>
            </div>
        }
    ...
    renderForm(files: { filename: string, size: number, url: string }[]) {
        var text = lst('添加文件');
    ...
    renderField(files: { filename: string, size: number, url: string }[]) {
        var text = lst('添加文件');
        var removeText = lst('移除文件')
        if (this.block.field?.type == FieldType.video) { text = lst('添加视频'); removeText = lst('移除视频') }
        else if (this.block.field?.type == FieldType.audio) { text = lst('添加音频'); removeText = lst('移除音频') }
