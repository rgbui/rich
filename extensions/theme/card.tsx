    ...
    renderCustom() {
        ...
        return <div className="padding-w-10">
            <div className="remark gap-b-5 f-12 flex">
                <span className="flex-auto"><S>布局</S></span>
                <span className="flex-fixed">
                    <HelpText url={window.shyConfig?.isUS ? "https://help.shy.red/page/76#6R5HYHohVAmbonKCMEHBco" : "https://help.shy.live/page/2093#tmr2Gt1vREB49XNSMZ129K"}><S>了解如何使用卡片块</S></HelpText>
                </span>
            </div>
            <div className="flex r-gap-r-10 r-round-2">
                <Tip text='无背景'><div className={'round padding-5 ' + ((this.pageTheme?.coverStyle?.display || 'none') == 'none' ? "item-hover-focus" : "item-hover")}>
                ...
                <Tip text='背景' ><div className={"round padding-5 " + (this.pageTheme?.coverStyle?.display == 'inside' ? " item-hover-focus" : "item-hover")}><div className="cursor round" onMouseDown={e => this.setTheme('coverStyle.display', 'inside')} style={{ border: this.pageTheme?.coverStyle?.display == 'inside' ? solid : '2px solid #cac5c4', width: 30, height: 20, background: this.pageTheme?.coverStyle?.display == 'inside' ? textp : '#cac5c4' }}></div></div></Tip>
                <Tip text='Top背景' ><div className={"round padding-5 " + (this.pageTheme?.coverStyle?.display == 'inside-cover' ? " item-hover-focus" : " item-hover")}><div className="cursor round" onMouseDown={e => this.setTheme('coverStyle.display', 'inside-cover')} style={{ border: this.pageTheme?.coverStyle?.display == 'inside-cover' ? solid : '2px solid #cac5c4', width: 30, height: 20 }}><div style={{ background: this.pageTheme?.coverStyle?.display == 'inside-cover' ? textp : '#cac5c4', height: 6 }}></div></div></div></Tip>
                <Tip text='Left背景'><div className={"round padding-5 " + (this.pageTheme?.coverStyle?.display == 'inside-cover-left' ? " item-hover-focus" : " item-hover")}><div className="cursor flex round" onMouseDown={e => this.setTheme('coverStyle.display', 'inside-cover-left')} style={{ border: this.pageTheme?.coverStyle?.display == 'inside-cover-left' ? solid : '2px solid #cac5c4', width: 30, height: 20 }} ><div style={{ background: this.pageTheme?.coverStyle?.display == 'inside-cover-left' ? textp : '#cac5c4', width: 10, height: 20 }}></div></div></div></Tip>
                <Tip text='Right背景' ><div className={"round padding-5 " + (this.pageTheme?.coverStyle?.display == 'inside-cover-right' ? " item-hover-focus" : " item-hover")}><div className="cursor flex-end round" onMouseDown={e => this.setTheme('coverStyle.display', 'inside-cover-right')} style={{ border: this.pageTheme?.coverStyle?.display == 'inside-cover-right' ? solid : '2px solid #cac5c4', width: 30, height: 20 }} ><div style={{ background: this.pageTheme?.coverStyle?.display == 'inside-cover-right' ? textp : '#cac5c4', width: 10, height: 20 }}></div></div></div></Tip>
            </div>
