        ...
        if (this.props.head || this.props.children || this.props.showSn) {
            return <div className={'shy-avatar-say'}>
                ...
                <div className={'shy-avatar-say-content'} >
                    <div className={'shy-avatar-say-content-head'}><div onMouseDown={e => this.mousedown(e)}><a >{this.user?.name}</a>{this.props.showSn && <span>#{this.user?.sn}</span>}</div>{this.props.head && <div>{this.props.head}</div>}</div>
